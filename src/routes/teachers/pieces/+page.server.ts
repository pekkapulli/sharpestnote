...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	const pieces = ((data ?? []) as TeacherPieceRow[]).map((piece) => {
		...
		let composerUrl = '/teachers/composer';
		try {
			composerUrl = `/teachers/composer?draft=${encodeURIComponent(packCustomUnitMaterialForUrl(piece.custom_unit_material as CustomUnitMaterial))}`;
		} catch {
			composerUrl = '/teachers/composer';
		}
